...
import OciRequestSigner from './oci-sign.js';

import { readFile } from 'node:fs/promises';
import { isAbsolute, join as joinPaths } from 'node:path';
import {homedir} from 'node:os';
import { loadLocalConfig } from './oci-config-loader.js';
...
class IamUtil {
  ...
  async #getSigner(instance){
    logger.debug("Generating a request signer for " +instance +"...");
    logger.debug("Reading config from '" +(this.config[instance].config_file || joinPaths(homedir(), '.oci', 'config'))+"'...");
    let config;
    try{
      config = loadLocalConfig(this.config[instance].config_file || joinPaths(homedir(), '.oci', 'config'), this.config[instance].profile || "default");
    }catch(e){
      logger.error("Error reading config from '"+this.config[instance].config_file || joinPaths(homedir(), '.oci', 'config')+"!");
      logger.error(e);
      return null;
    }
    logger.debug("Reading key from '" +config.key_file +"'...");
    let key;
    try{
      key = await readFile(config.key_file, {encoding: 'utf8'})
    }catch(e){
    ...
    return new OciRequestSigner(keyId, key, config.pass_phrase);
  }

  //Helper function to wrap up the fetch call add any required authentication
  async callIamEndpoint(instance, options) {
    ...
    options.url = this.config[instance].base_url.trim() + options.url;
    if (!options.headers) {
      options.headers = {};
    }
    //Check the Authentication Type
    //If Client Credentials, then insert our token, obtaining one if required.
    if (this.config[instance].auth_type === "client_credentials" || (this.config[instance].client_id && this.config[instance].client_secret)) {
    ...
    //If Request Signing, sign the request
    if (this.config[instance].auth_type === "config_file") {
      if (!this.signers[instance]){
        this.signers[instance] = await this.#getSigner(instance);
        if(!this.signers[instance]){
          throw new Error("Could not parse OCI Config file to create a request signer for " +instance);
        }
      }
      this.signers[instance].signRequest(options);
    ...
    try {
      let response = await fetch(encodeURI(options.url), options);
      if (response.ok) {
